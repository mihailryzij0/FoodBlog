export class Slider{
  constructor(selector, config){
   ...
   this.createDots(); 
    ...
    createDots(){
    if(this._config.doots == true){
      let listDoots = `<ul class= 'slider__dots-box' >`;
      for(let i = 0; i<this.sliderImg.length;i++){
        listDoots += `<li style= "flex-basis:${100 / this.sliderImg.length - 1}% " class= 'slider__dot'></li>`;
      }
      listDoots+=`</li>`
  ...
  autoplay(){
    if(this._config.autoplay == true ){
      let x = setInterval(()=> this.sliderNext(), this._config.interval);
